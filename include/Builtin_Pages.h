...
String OTAServerIndex =
    ...
    "</div>"
    "<div id='status' style='margin-top:8px;text-align:center;'></div>"
    "<div style='margin-top:6px;color:#888;font-size:12px;'>Valid files: firmware.bin or littlefs.bin</div>"
    "</div>"
    "</body>"
    "<script>"
    ...
    "prg.style.width = w;"
    "if(p.loaded===p.total){"
    "status.innerHTML = 'Flashing… please wait.';"
    "prg.style.backgroundColor='#f0a500';"
    "}"
    "}"
    "});"
    "req.onload = function(){"
    ...
    "prg.innerHTML='Done';"
    "status.innerHTML='Flash OK! Device is rebooting… reconnect in ~15 seconds.';"
    "}else{"
    "prg.style.backgroundColor='#cc0000';"
    "prg.innerHTML='Error';"
    "status.innerHTML='Flash FAILED (HTTP '+req.status+'). Try again.';"
    "}"
    "};"
    "req.onerror = function(){"
    "status.innerHTML='Connection lost (device may be rebooting). If the file was valid, reconnect in ~15 seconds.';"
    "};"
    "req.send(data);"
    "});"
    "</script>";
