...
8. ~~**Cadence scale factor config**~~ — DONE: `cadenceCorrectionFactor` (char 0x32, MIN_CCF/MAX_CCF, default 1.0) applied in `SensorCollector` at ingestion so FTMS broadcast, ERG gate, homing trigger, power-table keys and ShiftResponse all see one corrected value. A material change calls `powerTable->downgradeConfidence()`.
